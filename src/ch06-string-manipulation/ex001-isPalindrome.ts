...
/*
    ...
    - O(n)으로 해결가능. n은 s의 길이
 */
export function isPalindrome(s: string): boolean {
    // to normalized string
    const ns = s.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()
    let r1: number = 0,
...
/*
    # 해법 2
    해법 1의 성능 소폭 개선. 처음에 소문자로 일괄변환, 알파벳과 숫자를 제외한 모든 문자 제거하는 작업 역시 전체 문자에 대한 
    (동일) 소문자로 일괄변환, 알파벳과 숫자를 제외한 모든 문자 제거하는 작업역시 비용이므로 한 번에 진행하지 말고 비교로직에 포함시킨다
    (개선) 유효한 비교값인 /A-Za-z0-9/ 인 경우만 비교하고 그 외의 값이 올 경우는 인덱스를 이동시킨다 
    (동일) 2개가 만나거나 교차되면 종료한다 (runner1의 index >= runner2의 index)
    - O(n)으로 해결가능. n은 s의 길이
    -> 성능이 개선될 것으로 예상했지만 실제로 검사해보면 오히려 성능이 떨어지는걸 확인할 수 있다
 */
